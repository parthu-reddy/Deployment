...
echo "Building and restarting eureka-server-1 on remote..."
...
mvn clean package -pl EurekaServer -am -Pdev -Dmaven.test.skip=true

cd Deployment
export SPRING_PROFILES_ACTIVE=dev
docker compose build --no-cache eureka-server-1
docker compose up -d eureka-server-1
EOF

echo "Deployment of eureka-server-1 complete!"
