...
echo "Building and restarting maps-service on remote..."
...
mvn clean package -pl MapsIntegration -am -Pdev -Dmaven.test.skip=true </dev/null

cd Deployment
export SPRING_PROFILES_ACTIVE=dev
docker compose build --no-cache maps-service
docker compose up -d maps-service
EOF

echo "Deployment of maps-service complete!"
