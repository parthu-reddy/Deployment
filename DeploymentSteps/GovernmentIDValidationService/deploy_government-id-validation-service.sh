...
echo "Building and restarting government-id-validation-service on remote..."
...
mvn clean package -pl GovernmentIDValidationService -am -Pdev -Dmaven.test.skip=true </dev/null

cd Deployment
export SPRING_PROFILES_ACTIVE=dev
docker compose build --no-cache government-id-validation-service
docker compose up -d government-id-validation-service
EOF

echo "Deployment of government-id-validation-service complete!"
