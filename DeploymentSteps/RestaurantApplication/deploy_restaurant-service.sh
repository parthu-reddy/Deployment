...
# Ensure script is run from the DeploymentSteps/RestaurantApplication directory or a valid relative path
cd "$(dirname "$0")"
...
# Rebuild RestaurantApplication
mvn clean package -pl RestaurantApplication -am -Pdev -Dmaven.test.skip=true </dev/null

cd Deployment
export SPRING_PROFILES_ACTIVE=dev
docker compose build --no-cache restaurant-service
docker compose up -d restaurant-service
EOF

echo "Deployment of restaurant-service complete!"
