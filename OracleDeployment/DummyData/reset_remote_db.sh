...
ssh -o StrictHostKeyChecking=no -i "$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" \
    "cd '$COMPOSE_DIR' && docker compose stop customer-service restaurant-service delivery-service identity-service government-id-service payment-gateway communication-integration ledger-service"

for db in identity_db restaurant_db food_delivery delivery_db government_id_db payment_db notification_db ledger_db; do
...
ssh -o StrictHostKeyChecking=no -i "$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" \
    "cd '$COMPOSE_DIR' && docker compose start identity-service restaurant-service customer-service delivery-service government-id-service payment-gateway communication-integration ledger-service"

echo "Waiting for Spring Boot microservices to boot up and generate their database schemas..."
for db_table in "identity_db:users" "food_delivery:customers" "delivery_db:delivery_executives" "restaurant_db:brands" "government_id_db:executive_documents" "ledger_db:ledger_entries"; do
    db="${db_table%%:*}"
    table="${db_table##*:}"
